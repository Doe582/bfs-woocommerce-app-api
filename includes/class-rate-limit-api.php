...
/**
 * Class BFS_RateLimit_API
 *
 * Token-bucket style rate limiting using WP transients.
 * Defaults: 100 requests / 60 seconds per IP or user.
 *
 * Filters:
 *   bfs_rate_limit_enabled  (bool)   — disable completely
 *   bfs_rate_limit          (int)    — max requests per window
 *   bfs_rate_window         (int)    — window in seconds
 *   bfs_rate_limit_whitelist (array) — user IDs exempt from limiting
 */
class BFS_RateLimit_API {
    ...
    public static function check(\WP_REST_Request $req, string $action = 'default'): void {
        if (!apply_filters('bfs_rate_limit_enabled', true)) return;

        $user_id    = get_current_user_id();
        $whitelist  = (array) apply_filters('bfs_rate_limit_whitelist', []);

        if ($user_id && in_array($user_id, $whitelist, true)) return;

        $identifier = $user_id ? "user_{$user_id}" : 'ip_' . self::get_ip();
        $key        = 'bfs_rl_' . md5($identifier . '_' . $action);
        $limit      = (int) apply_filters('bfs_rate_limit', self::DEFAULT_LIMIT);
        $window     = (int) apply_filters('bfs_rate_window', self::DEFAULT_WINDOW);
